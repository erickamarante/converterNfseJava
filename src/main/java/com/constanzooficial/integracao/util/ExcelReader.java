...
public class ExcelReader {
    ...
    public String getConteudoCelulaString(int numeroCelula) {
        ...
        if (numeroCelula < this.linha.getFirstCellNum() || numeroCelula > this.linha.getLastCellNum()) {
            ...
            String retorno;
            if (this.linha.getCell(numeroCelula) != null) {
                switch (this.linha.getCell(numeroCelula).getCellTypeEnum()) {
                    case STRING:
                        ...
                        return retorno;
                    case NUMERIC:
                        retorno = Double.toString(this.linha.getCell(numeroCelula).getNumericCellValue());
                        if (retorno.endsWith(".0")) {
                            retorno = retorno.substring(0, retorno.length() - 2);
                        }
                        return retorno;
    ...
    public String buscaBinariaMunicipio(double codigoMunicipio) {
        ...
        if (codigoMunicipio == this.planilha.getRow(esquerda).getCell(0).getNumericCellValue()) {
            return this.planilha.getRow(esquerda).getCell(1).getStringCellValue();
        }
        
        while (esquerda <= direita) {
            meio = ((esquerda + direita) / 2) + ((esquerda + direita) % 2);
