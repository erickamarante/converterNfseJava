...
public class GeradorTXT {
    ...
    public GeradorTXT(ConsultarNfseFaixaResposta consultarNfseFaixaResposta) {
        this.listaNfse = consultarNfseFaixaResposta.getListaNfse().getCompNfse();
    }
    ...
    public void gerarTXT(String path, boolean considerarISS) throws IOException {
        ...
        if (!listaNfse.isEmpty()) {
            ...
            for (CompNfse compNfse : listaNfse) {
                ...
                int indicadorCpfCnpj;
                if (infNfse.getPrestadorServico().getIdentificacaoPrestador().getCpfCnpj().getCpf() != null) {
                    indicadorCpfCnpj = 1;
                    gravarArq.append("1");
                } else {
                    indicadorCpfCnpj = 2;
                ...
                gravarArq.append(strTamanhoMax((indicadorCpfCnpj == 1) ? infNfse.getPrestadorServico().getIdentificacaoPrestador().getCpfCnpj().getCpf() : infNfse.getPrestadorServico().getIdentificacaoPrestador().getCpfCnpj().getCnpj(), 14, "numerico"));
                ...
                // Opção Pelo Simples
                if (infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getOptanteSimplesNacional()) {
                    gravarArq.append('4');
                } else {
                    gravarArq.append('0');
                }
                ...
                if (compNfse.getNfseCancelamento() != null) {
                    if (compNfse.getNfseSubstituicao() != null) {
                        gravarArq.append('S');
                    } else {
                        gravarArq.append('C');
                    }
                    // Data de Cancelamento
                    data = compNfse.getNfseCancelamento().getConfirmacaoCancelamento().getDataHora();
                    gravarArq.append(data[0] + data[1] + data[2]);
                } else {
                ...
                // Valor dos Serviços
                double valor = Double.parseDouble(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getServico().getValores().getValorServicos());
                gravarArq.append(strTamanhoMax(doubleParaStringSemPontoCom2CasasDecimais(valor), 15, "numerico"));
                valorTotalServicos += valor;
                // Valor das Deduções
                valor = Double.parseDouble(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getServico().getValores().getValorDeducoes());
                gravarArq.append(strTamanhoMax(doubleParaStringSemPontoCom2CasasDecimais(valor), 15, "numerico"));
                valorTotalDeducoes += valor;
                // Código do Serviço Prestado na Nota Fiscal
                gravarArq.append(strTamanhoMax(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getServico().getItemListaServico(), 5, "numerico"));
                // Alíquota
                if (considerarISS) {
                    valor = Double.parseDouble(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getServico().getValores().getAliquota());
                } else {
                    valor = 0.0;
                }
                ...
                // Valor do ISS
                if (considerarISS) {
                    valor = Double.parseDouble(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getServico().getValores().getValorIss());
                } else {
                    valor = 0.0;
                }
                gravarArq.append(strTamanhoMax(doubleParaStringSemPontoCom2CasasDecimais(valor), 15, "numerico"));
                valorTotalISS += valor;
                // Valor do Crédito
                valor = Double.parseDouble(infNfse.getValorCredito());
                gravarArq.append(strTamanhoMax(doubleParaStringSemPontoCom2CasasDecimais(valor), 15, "numerico"));
                valorTotalCreditos += valor;
                // ISS Retido
                gravarArq.append((infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getServico().getIssRetido()) ? "S" : "N");
                // Indicador de CPF/CNPJ do Tomador
                if (infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getIdentificacaoTomador().getCpfCnpj().getCpf() != null) {
                    gravarArq.append("1");
                    // CPF ou CNPJ do Tomador
                    gravarArq.append(strTamanhoMax(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getIdentificacaoTomador().getCpfCnpj().getCpf(), 14, "numerico"));
                } else if (infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getIdentificacaoTomador().getCpfCnpj().getCnpj() != null) {
                    gravarArq.append("2");
                    // CPF ou CNPJ do Tomador
                    gravarArq.append(strTamanhoMax(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getIdentificacaoTomador().getCpfCnpj().getCnpj(), 14, "numerico"));
                } else {
                    gravarArq.append("3");
                    // CPF ou CNPJ do Tomador
                ...
                gravarArq.append(strTamanhoMax(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getRazaoSocial(), 75, "texto"));
                // Tipo do Endereço do Tomador (Rua, Av, ...)
                split = separaTipoEndereco(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getEndereco().getEndereco(), 3, 50);
                gravarArq.append(split[0]);
                // Endereço do Tomador
                gravarArq.append(split[1]);
                // Número do Endereço do Tomador
                gravarArq.append(strTamanhoMax(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getEndereco().getNumero(), 10, "texto"));
                // Complemento do Endereço do Tomador
                gravarArq.append(strTamanhoMax(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getEndereco().getComplemento(), 30, "texto"));
                // Bairro do Tomador
                gravarArq.append(strTamanhoMax(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getEndereco().getBairro(), 30, "texto"));
                // Cidade do Tomador
                codigoCidade = infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getEndereco().getCodigoMunicipio();
                if (codigoCidade.equals(codigoCidadeAnterior)) {
                    gravarArq.append(strTamanhoMax(cidadeAnterior, 50, "texto"));
                } else {
                    codigoCidadeAnterior = codigoCidade;
                ...
                // UF do Tomador
                gravarArq.append(strTamanhoMax(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getEndereco().getUf(), 2, "texto"));
                // CEP do Tomador
                gravarArq.append(strTamanhoMax(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getEndereco().getCep(), 8, "numerico"));
                // Email do Tomador
                if (infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getContato() != null) {
                    gravarArq.append(strTamanhoMax(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getTomador().getContato().getEmail(), 75, "texto"));
                } else {
                    gravarArq.append(strTamanhoMax("", 75, "texto"));
                }
                // Discriminação dos Serviços
                gravarArq.append(infNfse.getDeclaracaoPrestacaoServico().getInfDeclaracaoPrestacaoServico().getServico().getDiscriminacao().replace("<br /><br />", "; ").replaceAll("<br />", "; "));
                // Caractere de Fim de Linha
                gravarArq.append("\n");
