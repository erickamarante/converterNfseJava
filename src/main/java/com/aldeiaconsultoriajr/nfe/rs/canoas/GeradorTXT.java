...
import static com.constanzooficial.integracao.util.MyUtils.strTamanhoMax;
    ...
    public void gerarTXT(String path, boolean considerarISS) throws IOException {
        ...
        if (!listaNfse.isEmpty()) {
            ...
            for (CompNfse compNfse : listaNfse) {
                ...
                if (infNfse.getOutrasInformacoes().toLowerCase().contains("cancelada")) {
                    gravarArq.append("C");
                    cancelada = true;
                } else {
                    gravarArq.append(" ");
                ...
                if (cancelada) {
                    String tmp = infNfse.getOutrasInformacoes().substring(infNfse.getOutrasInformacoes().toLowerCase().lastIndexOf("cancelada"));
                    split = tmp.split("/");
                    String dia = split[0].substring(split[0].length() - 2).replaceAll("[^0-9]", "");
                    dia = strTamanhoMax(dia, 2, "numerico");
                    String mes = strTamanhoMax(split[1], 2, "numerico");
                    String ano = strTamanhoMax(split[2].substring(0, 4).replaceAll("[^0-9]", ""), 4, "numerico");
                    gravarArq.append(ano + mes + dia);
                } else {
                    gravarArq.append("        ");
                }
                // Nº da Guia
                ...
                if (considerarISS) {
                    ...
                    gravarArq.append(strTamanhoMax(null, 15, "numerico"));
                }
                // Valor do Crédito
